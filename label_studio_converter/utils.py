...
def parse_local_uri_to_image_path(uri):
    """ 
      Parse local uri generated by a server hosting files that uses compresion and base64 to represent 
      image path locally.
    """
    image_uri = urlparse(uri).path
    image_id_start = image_uri.rfind('/')
    image_id = image_uri if image_uri == -1 else image_uri[image_id_start + 1:]
    if '.' not in image_id:
        return None
    image_id = image_id.split('.')[1]
    try:
        return Path(zlib.decompress(base64.urlsafe_b64decode(image_id)).decode('utf-8'))
    except Exception as e:
        logger.warning('Unable to parse uri to local image path. ' + str(uri) + '\n' + str(e))
    return None    
...
def download(
    ...
    if filename is None:
        image_path = parse_local_uri_to_image_path(url)
        if image_path:
